# Operating Systems Lab Examination - Comprehensive Preparation Guide
...
> - **Question 1:** 10 Marks
> - **Question 2:** 15 Marks
> - **Syllabus Constraint:** Strictly POSIX / Linux System Calls discussed in lab; **No xv6 questions**.
...
> Addresses the prompt from [week3/q2](file:///c:/Users/moodm/Desktop/IIITDMK/Studies/Semester%205/Operating%20Systems/LAB(av)/week3/q2): Two pipes for two-way communication, graceful error handling for division by zero, all unused ends closed.
...
> Addresses the prompt from [week4/q1](file:///c:/Users/moodm/Desktop/IIITDMK/Studies/Semester%205/Operating%20Systems/LAB(av)/week4/q1): Modern POSIX API with `/lab_shm_basic`, 256 bytes buffer, inspectable via `/dev/shm`.
...
```
* **Compilation:** `gcc posix_sender.c -o posix_sender -lrt` and `gcc posix_receiver.c -o posix_receiver -lrt`.

---

### 3.3 System V Shared Memory (`ftok`, `shmget`, `shmat`)
> Addresses the prompt from [week4/q2](file:///c:/Users/moodm/Desktop/IIITDMK/Studies/Semester%205/Operating%20Systems/LAB(av)/week4/q2): Key generation via `ftok(".", 'B')`, attaching, detaching, inspecting via `ipcs -m`, and eviction via `shmctl IPC_RMID`.
...
> Addresses the prompt from [week4/q3](file:///c:/Users/moodm/Desktop/IIITDMK/Studies/Semester%205/Operating%20Systems/LAB(av)/week4/q3): Lock-free parallel multiplication $C = A \times B$ across $K$ workers.
...
> Pattern from [week2/q2](file:///c:/Users/moodm/Desktop/IIITDMK/Studies/Semester%205/Operating%20Systems/LAB(av)/week2/q2/copy.c) and [week2/q3](file:///c:/Users/moodm/Desktop/IIITDMK/Studies/Semester%205/Operating%20Systems/LAB(av)/week2/q3/controller.c):
...
#### A. Zombie Process Demonstration ([week1/q4](file:///c:/Users/moodm/Desktop/IIITDMK/Studies/Semester%205/Operating%20Systems/LAB(av)/week1/q4/q4.c))
```c
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
#### B. Orphan Detection without `sleep()` ([week2/q4](file:///c:/Users/moodm/Desktop/IIITDMK/Studies/Semester%205/Operating%20Systems/LAB(av)/week2/q4/q4.c))
```c
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
* **Statement:** Write a C program using two pipes (`p2c` and `c2p`) where the parent sends an arbitrary string entered by the user to the child process. The child:
  1. Counts vowels and consonants.
  2. Reverses the string.
  3. Sends both the count and reversed string back to the parent.
  The parent displays the received values, closes all pipes cleanly, and calls `wait()`.
...
* **Statement:** An array of $N$ integers ($N = 1000$ or user input) is stored in shared memory. Write a program where the parent allocates shared memory for the array and a results array of size $K$. 
  - The parent spawns $K$ worker children.
  - Worker $w$ computes the partial sum (or local maximum) of elements between index $w \cdot \frac{N}{K}$ and $(w + 1) \cdot \frac{N}{K} - 1$, and stores the value in `results[w]`.
  - The parent waits for all $K$ workers to terminate, aggregates the final answer from `results[]`, compares it with a sequential verification loop, and unlinks/destroys the shared memory.
...
* If a program crashes before calling `shmctl(shmid, IPC_RMID, NULL)`, the segment stays resident in the Linux kernel.
* Inspect:
  ```bash
  ipcs -m
  ```
